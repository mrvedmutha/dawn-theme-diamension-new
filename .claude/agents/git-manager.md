...
**Goal**: Clean merge to main with comprehensive documentation
...
3. Merge with squash (keeps history clean): `git merge --squash feature/section/[section-name]`
4. Create comprehensive final commit:
   ```
   New Section: [Display Name], Development Completed
   ...
   ```
5. Capture final commit hash: `git rev-parse HEAD`
6. Delete feature branch (cleanup): `git branch -D feature/section/[section-name]`
7. Update state.json:
   ```json
   {
...
5. **ALWAYS stage only section files** - Use explicit `git add` commands, never `git add .`
