/**
 * Sleek Minimal Hero Section - Scroll Animation
...
(function() {
  ...
  function initScrollAnimation() {
    const sections = document.querySelectorAll('.custom-section-sleek-minimal-hero');
      ...
      const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
          ...
          if (entry.isIntersecting && entry.intersectionRatio >= 0.2) {
            ...
            setTimeout(() => {
              section.classList.add('custom-section-sleek-minimal-hero--animated');
            }, 400);
    ...
    window.addEventListener('resize', () => {
      ...
      resizeTimer = setTimeout(() => {
        ...
        if (window.innerWidth < 768) {
          sections.forEach(section => {
            section.classList.remove('custom-section-sleek-minimal-hero--animated');
          });
        }
      }, 250);
    });
