/**
 * Image Hero Section - Corporate - Scroll Animation
...
(function() {
  ...
  function initScrollAnimation() {
    const sections = document.querySelectorAll('.custom-section-image-hero-corporate');
      ...
      const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
          ...
          if (entry.isIntersecting && entry.intersectionRatio >= 0.2) {
            ...
            setTimeout(() => {
              section.classList.add('custom-section-image-hero-corporate--animated');
            }, 400);
    ...
    window.addEventListener('resize', () => {
      ...
      resizeTimer = setTimeout(() => {
        ...
        if (window.innerWidth < 768) {
          sections.forEach(section => {
            section.classList.remove('custom-section-image-hero-corporate--animated');
          });
        }
      }, 250);
    });
